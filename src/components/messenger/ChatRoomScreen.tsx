...
import CallOverlay from './CallOverlay'; // কল চলাকালীন স্ক্রিন (যদি আগে বানিয়ে থাকো)

interface ChatRoomProps {
  receiver: any;
  onBack: () => void;
  onViewProfile: (userId: string) => void;
  onStartCall: (isVideo: boolean) => void; // ✅ নতুন প্রপ: কল শুরু করার জন্য
}
...
const ChatRoomScreen: React.FC<ChatRoomProps> = ({ receiver, onBack, onViewProfile, onStartCall }) => {
  ...
  const sendMessage = async (e?: React.FormEvent) => {
    ...
    try {
        ...
        const msg = {
            sender_id: user?.id, receiver_id: receiver.id, content: msgContent,
        ...
        await supabase.from('messages').insert({
            sender_id: user?.id, receiver_id: receiver.id, content: msgContent, image_url: uploadedImageUrl
        });
    ...
    <div className="flex flex-col h-screen bg-gray-50 dark:bg-gray-900 fixed inset-0 z-50 transition-colors">
      <div className="bg-white dark:bg-gray-800 p-3 shadow-sm flex items-center justify-between border-b dark:border-gray-700">
        <div className="flex items-center gap-3">
            <button onClick={onBack} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full dark:text-white transition-colors"><ArrowLeft size={22} /></button>
            <div className="flex items-center gap-3 cursor-pointer" onClick={() => onViewProfile(receiver.id)}>
                <div className="relative">
                    ...
                    {isOnline && <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-white dark:border-gray-800 rounded-full animate-pulse"></div>}
                </div>
                <div>
                    <h3 className="font-bold text-sm dark:text-white hover:underline">{receiver.name}</h3>
                    <span className={`text-xs ${isOnline ? 'text-green-600 font-medium' : 'text-gray-400'}`}>{isOnline ? 'Active now' : 'Offline'}</span>
                </div>
            </div>
        </div>
        ...
        <div className="flex gap-4 text-blue-600 dark:text-blue-400 pr-2">
            <button onClick={() => onStartCall(false)} className="hover:bg-blue-50 dark:hover:bg-gray-700 p-2 rounded-full"><Phone size={22} /></button>
            <button onClick={() => onStartCall(true)} className="hover:bg-blue-50 dark:hover:bg-gray-700 p-2 rounded-full"><Video size={24} /></button>
        </div>
      </div>
        ...
        {messages.map((msg, idx) => {
            ...
            return (
                <div key={idx} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[75%] rounded-2xl shadow-sm overflow-hidden ${isMe ? 'bg-blue-600 text-white rounded-br-none' : 'bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 border dark:border-gray-700 rounded-bl-none'}`}>
                        {msg.image_url && <img src={msg.image_url} className="w-full h-auto max-h-60 object-cover" />}
                        {msg.content && <div className="px-4 py-2 text-sm break-words">{msg.content}</div>}
                    </div>
                </div>
            );
        })}
      ...
      <div className="p-3 bg-white dark:bg-gray-800 border-t dark:border-gray-700 relative">
        ...
        <form onSubmit={sendMessage} className="flex items-center gap-2 bg-gray-100 dark:bg-gray-700 p-1.5 rounded-3xl transition-colors">
            ...
            <button type="button" onClick={() => fileInputRef.current?.click()} className="p-2 text-gray-500 hover:text-green-500"><Image size={24} /></button>
            <input type="file" ref={fileInputRef} hidden accept="image/*" onChange={handleImageSelect} />
            <input value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Type a message..." className="flex-1 bg-transparent px-2 py-2 focus:outline-none text-sm dark:text-white placeholder-gray-500 dark:placeholder-gray-400" />
            <button type="submit" disabled={(!newMessage.trim() && !selectedImage) || sending} className="p-2 bg-blue-600 text-white rounded-full hover:bg-blue-700 disabled:opacity-50 active:scale-95 flex justify-center w-10 h-10">
